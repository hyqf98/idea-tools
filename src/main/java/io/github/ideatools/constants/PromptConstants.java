...
package io.github.ideatools.constants;
...
public final class PromptConstants {
    ...
    public static final String DEFAULT_CLASS_PROMPT = """
            ...
            * 这是一个用于处理用户信息的服务类，\n            * 提供用户注册、登录、信息修改、\n            * 密码找回等功能
            ...
            【模板变量替换规则 - 非常重要】
            请根据下方【上下文参数】中的值替换模板中的占位符：
            - ${description} 替换为：你根据代码生成的功能描述
            - ${author} 替换为：上下文参数中的 author 值
            - ${date} 替换为：上下文参数中的 date 值
            - ${version} 替换为：上下文参数中的 version 值
            - ${since} 替换为：上下文参数中的 since 值
            - ${email} 替换为：上下文参数中的 email 值（保留双引号格式，如 "mailto:xxx@email.com"）
            - #if($email && $email != "") ... #end 部分如果 email 不为空则保留，否则删除整段
            - #foreach($param in $parameters) ... #end 部分根据 parameters 数组循环展开

            注释模板：
            {template}

            上下文参数（请使用这些值替换模板中的占位符）：
            {context}
    ...
    public static final String DEFAULT_METHOD_PROMPT = """
            ...
            * 根据用户ID和权限类型查询数据库，\n            * 返回用户的所有权限信息列表
            ...
            请根据下方【上下文参数】中的值替换模板中的占位符：
            - ${description} 替换为：你根据代码生成的功能描述
            - ${returnType} 和 ${returnType.splitName} 替换为：上下文参数中的返回值信息
            - ${parameters} 替换为：方法参数列表
            - ${since} 替换为：上下文参数中的 since 值
            - #foreach($param in $parameters) ... #end 部分根据 parameters 数组循环展开
            - #if($returnType && $returnType != "") ... #end 部分如果有返回值则保留，否则删除整段
    ...
    public static final String DEFAULT_FIELD_PROMPT = """
            ...
            * 用户的唯一标识符，\n            * 用于在数据库中唯一标识一个用户记录，\n            * 自增长型
            ...
            【模板变量替换规则 - 非常重要】
            请根据下方【上下文参数】中的值替换模板中的占位符：
            - ${fieldName} 替换为：上下文参数中的 fieldName 值
            - ${util.camelToWords($fieldName)} 替换为：字段名转换后的中文描述
     ...
     * 包注释生成默认提示词模板
     */
    public static final String DEFAULT_PACKAGE_PROMPT = """
            请根据注释模板以及上下文参数生成标准的JavaDoc注释。
            ...
            【内容要求】
            请根据包名信息生成包注释，填充到模板的${description}位置：
            1. 第一段：用一句话说明该包的作用和包含的内容（注意50字符限制）
            2. 第二段（<p>后）：列举该包的主要功能或包含的类类型，每个特性单独一行，使用 "- " 开头
            3. 第三段（<p>后）：如果适用，提供简单的使用说明

            【模板变量替换规则 - 非常重要】
            请根据下方【上下文参数】中的值替换模板中的占位符：
            - ${description} 替换为：你根据包名生成的功能描述
            - ${author} 替换为：上下文参数中的 author 值
            - ${date} 替换为：上下文参数中的 date 值
            - ${version} 替换为：上下文参数中的 version 值
            - ${since} 替换为：上下文参数中的 since 值
            - ${email} 替换为：上下文参数中的 email 值（保留双引号格式，如 "mailto:xxx@email.com"）
            - ${packageName} 替换为：上下文参数中的 packageName 值
            - #if($email && $email != "") ... #end 部分如果 email 不为空则保留，否则删除整段
    ...
    /**
     * Python类docstring生成默认提示词模板
     */
    public static final String DEFAULT_PYTHON_CLASS_PROMPT = """
            请根据注释模板以及上下文参数生成标准的Python docstring注释。
            
            【重要输出要求】
            1. 必须使用UTF-8编码输出，直接输出中文字符
            2. 严格按照Google风格的docstring格式输出
            3. 使用三引号(\"\"\"或''')包围docstring内容
            4. 只返回docstring内容，不要包含代码块标记（```python等）
            5. 不要包含class定义，只返回docstring部分
            
            【格式规范 - Google Style】
            - 第一行：简短的类功能概述（一句话）
            - 空一行
            - Attributes部分：列出类的所有属性
            - 每个属性格式：属性名 (类型): 属性描述
            - 属性描述简洁明了，一般一行完成
            
            【描述长度规则】
            1. **首行概述**: 不超过80个字符（含中英文）
            2. **属性描述**: 每行不超过80个字符
            3. **自然换行**: 描述过长时在合适位置换行（标点符号后）
            4. **缩进对齐**: 所有内容保持4空格缩进
            ...
            【内容要求】
            请根据代码信息生成docstring内容：
            1. 首行：用一句话概括类的核心功能
            2. Attributes部分：
               - 列出所有类属性（实例变量和类变量）
               - 每个属性包含：名称、类型、简短描述
               - 类型使用Python标准类型（int, str, list, dict等）
            3. 保持简洁专业，避免冗余描述
    ...
    /**
     * Python函数docstring生成默认提示词模板
     */
    public static final String DEFAULT_PYTHON_FUNCTION_PROMPT = """
            请根据注释模板以及上下文参数生成标准的Python docstring注释。
            ...
            3. 使用三引号(\"\"\"或''')包围docstring内容
            4. 只返回docstring内容，不要包含代码块标记（```python等）
            5. 不要包含def定义，只返回docstring部分
            
            【格式规范 - Google Style】
            - 第一行：简短的函数功能概述（一句话）
            - 空一行（如果有Args/Returns/Raises）
            - Args部分：列出所有参数（除self和cls）
            - Returns部分：描述返回值类型和含义
            - Raises部分：列出可能抛出的异常（如果有）
            - 每个参数格式：参数名 (类型): 参数描述
            ...
            Args:
                a (int): 第一个加数
                b (int): 第二个加数
                precision (int, optional): 精度位数，默认为2
            
            Returns:
                int: 两数之和
            
            Raises:
                ValueError: 当参数不是数字时抛出
            \"\"\"
